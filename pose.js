...
let _vadSmoothed = 0;
let talking = false;
// Menu toggles (defaults may be overridden by head.json)
let speakTrackingEnabled = true;
let tiltTrackingEnabled = true;
let turnTrackingEnabled = true;
// track last direction used for head selection so blink overlays still match
let _lastHeadDirection = 'center';
...
// Talking image (shown only while voice activity detected)
const talkingImg = new Image(); talkingImg.src = 'images/talking.png'; let talkingLoaded=false; talkingImg.onload=()=>talkingLoaded=true;
// Room/background image (drawn when camera is off). Use backgrounds/room.png by default to avoid 404s.
const roomImg = new Image(); roomImg.src = 'images/backgrounds/room.png'; let roomLoaded=false; roomImg.onload=()=>roomLoaded=true;
roomImg.onerror = ()=>{ try{ roomImg.onerror = null; roomImg.src = 'images/room.png'; }catch(e){} };
...
let headConfig = {enabled:true,widthMultiplier:0.7,heightMultiplier:0.7,offsetX:0,offsetY:-10,rotationOffsetDegrees:0,alpha:1.0,leftImage:'images/headleft.png',rightImage:'images/headright.png',turnThreshold:0.18,flipVertical:false,flipHorizontal:false,blinkEnabled:true,blinkMinMs:2500,blinkMaxMs:6000,blinkDurationMs:120,blinkLeftImage:'images/blinkleft.png',blinkRightImage:'images/blinkright.png',tiltMultiplier:1.0,tiltThresholdDegrees:6,talkingImage:'images/talking.png',talkingThreshold:0.02,backgroundImage:'images/room.png',speakTrackingEnabled:true,tiltTrackingEnabled:true,turnTrackingEnabled:true};
// Scale config: prevents images from shrinking past a fraction of the initial shoulder distance
let scaleConfig = { minScaleFactor: 0.35 };
...
async function reloadConfigs(){
  ...
  // load optional scale config which controls minimal shoulder-based scaling
  try{
    const sResp = await fetch('scale.json');
    if (sResp.ok){ const cfg = await sResp.json(); Object.assign(scaleConfig, cfg); console.log('scale.json loaded', scaleConfig); }
  }catch(err){ console.warn('scale.json failed to load or parse', err); }
  ...
  try{
    ...
    // Update talking image if config provides a custom path
    if (headConfig.talkingImage){ talkingImg.src = headConfig.talkingImage; }
    // Update background image if config provides a custom path (fallback to backgrounds/room.png on error)
    if (headConfig.backgroundImage){
      roomImg.onerror = ()=>{ try{ roomImg.onerror = null; roomImg.src = 'images/backgrounds/room.png'; }catch(e){} };
      roomImg.src = headConfig.backgroundImage;
    }
    // Respect speak/tilt defaults from config and update menu checkboxes if present
    if (typeof headConfig.speakTrackingEnabled !== 'undefined') speakTrackingEnabled = !!headConfig.speakTrackingEnabled;
    if (typeof headConfig.tiltTrackingEnabled !== 'undefined') tiltTrackingEnabled = !!headConfig.tiltTrackingEnabled;
    if (typeof headConfig.turnTrackingEnabled !== 'undefined') turnTrackingEnabled = !!headConfig.turnTrackingEnabled;
    const ds = document.getElementById('disableSpeakChk'); if (ds) ds.checked = !speakTrackingEnabled;
    const dt = document.getElementById('disableTiltChk'); if (dt) dt.checked = !tiltTrackingEnabled;
    const dturn = document.getElementById('disableTurnChk'); if (dturn) dturn.checked = !turnTrackingEnabled;
  }catch(e){}
}
// initial load
reloadConfigs();
...
function drawUpperBody(landmarks){
  ...
  // If camera preview is hidden or no stream present, draw room background behind the character
  if ((!showCamera || !stream) && roomLoaded){
    try{ canvasCtx.drawImage(roomImg, 0, 0, canvasElement.width, canvasElement.height); }catch(e){}
  }
  ...
  // record initial shoulder distance once (used to compute scale thresholds)
  if (!_initialShoulderDist && shoulderDist > 0) _initialShoulderDist = shoulderDist;
  const minFactor = (scaleConfig && scaleConfig.minScaleFactor) ? Number(scaleConfig.minScaleFactor) : 0.35;
  const minShoulder = (_initialShoulderDist && minFactor) ? (_initialShoulderDist * minFactor) : shoulderDist;
  const effectiveShoulderDist = Math.max(shoulderDist, minShoulder);
  // skeleton offsets (pixels) from scaleConfig — apply to skeleton drawing and image positions
  const skeletonOffsetX = (scaleConfig && !isNaN(Number(scaleConfig.skeletonOffsetX))) ? Number(scaleConfig.skeletonOffsetX) : 0;
  const skeletonOffsetY = (scaleConfig && !isNaN(Number(scaleConfig.skeletonOffsetY))) ? Number(scaleConfig.skeletonOffsetY) : 0;
  ...
  if (torsoLoaded && torsoConfig.enabled && pts.lShoulder.v>0.2 && pts.rShoulder.v>0.2 && showImages){
    ...
    const inferredMidHip = { x: midShoulder.x, y: midShoulder.y + effectiveShoulderDist * (torsoConfig.hipOffsetMultiplier||1.6) };
    ...
    const torsoW = Math.max(20, effectiveShoulderDist * (torsoConfig.widthMultiplier||1.4));
    const cx = (midShoulder.x + inferredMidHip.x)/2 + (torsoConfig.centerOffsetX||0) + skeletonOffsetX;
    const cy = (midShoulder.y + inferredMidHip.y)/2 + (torsoConfig.centerOffsetY||0) + skeletonOffsetY;
    ...
    // Mirror handling: when preview is mirrored, flip the rotation sign instead
    const adjTorsoAngle = flipHorizontal ? -angle : angle;
  ...
  if (showSkeleton){
    ...
    // Allow global skeleton offset tweaks from scaleConfig (in pixels).
    const skeletonOffsetX = (scaleConfig && !isNaN(Number(scaleConfig.skeletonOffsetX))) ? Number(scaleConfig.skeletonOffsetX) : 0;
    const skeletonOffsetY = (scaleConfig && !isNaN(Number(scaleConfig.skeletonOffsetY))) ? Number(scaleConfig.skeletonOffsetY) : 0;
    const drawLine = (a,b,color) => { if (a && b && a.v>0.2 && b.v>0.2){ canvasCtx.strokeStyle=color; canvasCtx.beginPath(); canvasCtx.moveTo(a.x + skeletonOffsetX, a.y + skeletonOffsetY); canvasCtx.lineTo(b.x + skeletonOffsetX, b.y + skeletonOffsetY); canvasCtx.stroke(); } };
    ...
    const drawPt = (p) => { if (p && p.v>0.2){ const px = p.x + skeletonOffsetX, py = p.y + skeletonOffsetY; canvasCtx.fillStyle='#e6eef3'; canvasCtx.beginPath(); canvasCtx.arc(px,py,6,0,Math.PI*2); canvasCtx.fill(); canvasCtx.fillStyle='#0b1220'; canvasCtx.beginPath(); canvasCtx.arc(px,py,3,0,Math.PI*2); canvasCtx.fill(); } };
  ...
  if ((headLoaded || headLeftLoaded || headRightLoaded) && headConfig.enabled && pts.lShoulder && pts.rShoulder && showImages){
    ...
    const headW = Math.max(20, effectiveShoulderDist * (headConfig.widthMultiplier||0.7));
    ...
    const cx = midShoulder.x + (headConfig.offsetX||0) + skeletonOffsetX;
    ...
    const cy = midShoulder.y + offsetYPixels + skeletonOffsetY;
    ...
    if (!turnTrackingEnabled){
      // turn tracking disabled — always use center/head image when available
      chosenImg = headLoaded ? headImg : (headLeftLoaded ? headLeftImg : (headRightLoaded ? headRightImg : null));
      _lastHeadDirection = 'center';
    } else if (useNose && hasEyes){
      ...
      if (norm < -turnThreshold && headLeftLoaded) { chosenImg = headLeftImg; _lastHeadDirection = 'left'; }
      else if (norm > turnThreshold && headRightLoaded) { chosenImg = headRightImg; _lastHeadDirection = 'right'; }
      else { chosenImg = headLoaded ? headImg : (headLeftLoaded ? headLeftImg : (headRightLoaded ? headRightImg : null)); _lastHeadDirection = 'center'; }
    } else if (useNose){
      // fallback to shoulder-mid method
      const thresholdPx = shoulderDist * turnThreshold;
      if (pts.nose.x < midShoulder.x - thresholdPx && headLeftLoaded) { chosenImg = headLeftImg; _lastHeadDirection = 'left'; }
      else if (pts.nose.x > midShoulder.x + thresholdPx && headRightLoaded) { chosenImg = headRightImg; _lastHeadDirection = 'right'; }
      else { chosenImg = headLoaded ? headImg : (headLeftLoaded ? headLeftImg : (headRightLoaded ? headRightImg : null)); _lastHeadDirection = 'center'; }
    } else {
      chosenImg = headLoaded? headImg : (headLeftLoaded? headLeftImg : (headRightLoaded? headRightImg : null));
      // best-effort set direction
      if (chosenImg === headLeftImg) _lastHeadDirection = 'left';
      else if (chosenImg === headRightImg) _lastHeadDirection = 'right';
      else _lastHeadDirection = 'center';
    }

    if (chosenImg){
      // If voice activity detected and talking image is available and enabled, prefer it
      if (talking && talkingLoaded && speakTrackingEnabled) {
        chosenImg = talkingImg;
      }
      ...
      // Mirror handling: when preview is mirrored, flip the rotation sign instead
      const adjAngle = flipHorizontal ? -angle : angle;
      // Compute head roll (tilt) from eye landmarks and apply as additional rotation
      let headRoll = 0;
      try{
        const leftEyeIdx = 2, rightEyeIdx = 5;
        const leftEye = sm[leftEyeIdx], rightEye = sm[rightEyeIdx];
        if (leftEye && rightEye && (leftEye.v ?? 0) > 0.12 && (rightEye.v ?? 0) > 0.12){
          const eyeAngle = Math.atan2(rightEye.y - leftEye.y, rightEye.x - leftEye.x); // radians
          const thresholdRad = (Number(headConfig.tiltThresholdDegrees||6) * Math.PI/180);
          if (Math.abs(eyeAngle) > thresholdRad) headRoll = eyeAngle;
        }
      }catch(e){ headRoll = 0; }
      const tiltMul = Number(headConfig.tiltMultiplier||1);
      // Respect user toggle for tilt tracking
      const headAngleWithTilt = adjAngle + (tiltTrackingEnabled ? headRoll * tiltMul : 0);
      drawImageRotated(chosenImg, cx, cy, headW, headH, headAngleWithTilt, flipX, flipY);
      canvasCtx.globalAlpha = 1.0;
    }
    // Blinking: randomized automatic blink overlay drawn on top of the head
    try{
      const now = performance.now();
      if (headConfig.blinkEnabled && (blinkLoaded || blinkLeftLoaded || blinkRightLoaded) && showImages){
        ...
        if (now < (_blinkEndTime||0)){
          canvasCtx.globalAlpha = (typeof headConfig.alpha==='number')?headConfig.alpha:1.0;
          // Prefer directional blink overlays if available and the last known head direction
          let blinkToUse = blinkImg;
          if (_lastHeadDirection === 'left' && blinkLeftLoaded) blinkToUse = blinkLeftImg;
          else if (_lastHeadDirection === 'right' && blinkRightLoaded) blinkToUse = blinkRightImg;
          else if (blinkLoaded) blinkToUse = blinkImg;
          drawImageRotated(blinkToUse, cx, cy, headW, headH, headAngleWithTilt, flipX, flipY);
          canvasCtx.globalAlpha = 1.0;
        }
      }
...
async function startCamera(){
  ...
  try{
    ...
    const constraints = { audio: { echoCancellation: true }, video: {
    ...
    if (cameraErrorEl) { cameraErrorEl.style.display = 'none'; cameraErrorEl.textContent = ''; }

    // Set up WebAudio analyser for simple voice activity detection (VAD)
    try{
      ...
      _vadInterval = setInterval(()=>{
        try{
          ...
          let sum = 0;
          for (let i=0;i<audioDataArray.length;i++){ const v = (audioDataArray[i] - 128) / 128; sum += v*v; }
          const rms = Math.sqrt(sum / audioDataArray.length);
          _vadSmoothed = (_vadSmoothed * 0.85) + (rms * 0.15);
          const thresh = Number(headConfig.talkingThreshold || 0.02);
          talking = _vadSmoothed > thresh;
...
function showCameraErrorBanner(msg){
  if (cameraErrorEl){
    ...
    cameraErrorEl.innerHTML = '<strong>Camera unavailable:</strong> ' + (msg || 'This environment does not allow camera access.');
  }
}

// Patch: if startCamera failed earlier, wrap existing try/catch to display banner
// (Note: startCamera already logs errors to console; show banner for non-console contexts like OBS)
// Attach to window.onerror for extra safety
window.addEventListener('error', (ev)=>{
  // If an error occurs and no stream, inform the user
  if (!stream) showCameraErrorBanner(ev.message || 'Unknown error');
});
}
...
function stopCamera(){
  ...
  stream=null;
  // Tear down audio/VAD resources
  try{ if (_vadInterval) { clearInterval(_vadInterval); _vadInterval = null; } }catch(e){}
  try{ if (audioContext){ audioContext.close(); } }catch(e){}
  analyser = null; audioDataArray = null; audioContext = null; talking = false; _vadSmoothed = 0;
...
// Speak/tilt disable checkboxes
const disableSpeakChk = document.getElementById('disableSpeakChk');
if (disableSpeakChk){
  // checked = disabled
  disableSpeakChk.checked = !speakTrackingEnabled;
  disableSpeakChk.addEventListener('change', e=>{ speakTrackingEnabled = !e.target.checked; });
}
const disableTiltChk = document.getElementById('disableTiltChk');
if (disableTiltChk){
  // checked = disabled
  disableTiltChk.checked = !tiltTrackingEnabled;
  disableTiltChk.addEventListener('change', e=>{ tiltTrackingEnabled = !e.target.checked; });
}
...
// Turn-disable checkbox (speak/tilt handlers are declared earlier)
const disableTurnChk = document.getElementById('disableTurnChk');
if (disableTurnChk){
  disableTurnChk.checked = !turnTrackingEnabled;
  disableTurnChk.addEventListener('change', e=>{ turnTrackingEnabled = !e.target.checked; });
}
